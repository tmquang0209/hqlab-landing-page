	...
	const teamList: TeamProps[] = [
		{
			...
			socialNetworks: [
				{
					name: "LinkedIn",
					url: "https://www.linkedin.com/in/tmquang0209/",
				},
				{
					name: "Github",
					url: "https://github.com/tmquang0209",
				},
			],
		},
		{
			...
			socialNetworks: [
				...
				{
					name: "Github",
					url: "https://github.com/drawtok",
				},
			],
		},
		{
			imageUrl: "https://avatars.githubusercontent.com/u/126040951?v=4",
			firstName: "Hoang",
			lastName: "Nguyen",
			...
			socialNetworks: [
				...
				{
					name: "Github",
					url: "https://github.com/huuhoang150424",
				},
			],
		},
	];
				...
				{teamList.map(({ imageUrl, firstName, lastName, positions, socialNetworks }, index) => (
					<Card key={`${firstName}-${lastName}`} className="bg-muted/60 dark:bg-card flex flex-col h-full overflow-hidden group/hoverimg">
						...
						{positions.map((position, index) => (
							<CardContent key={`${firstName}-${lastName}-${position}`} className={`pb-0 text-muted-foreground ${index === positions.length - 1 && "pb-6"}`}>
								{position}
						...
						<CardFooter className="space-x-4 mt-auto">
							{socialNetworks.map(({ name, url }, index) => (
								<Link key={`${firstName}-${lastName}-${name}`} href={url} target="_blank" className="hover:opacity-80 transition-all">
									{socialIcon(name)}
								</Link>
							))}
						</CardFooter>
